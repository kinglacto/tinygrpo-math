...
#SBATCH --job-name=tinygrpo-no-kl
...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python3 scripts/tinygrpo_math.py \
  ...
  --train_size 500 \
  --learning_rate 5e-6 \
  --output_dir "$run_root/no_kl/stage1"

PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python3 scripts/tinygrpo_math.py \
  --mode train \
  "${common[@]}" \
  --init_model_path "$run_root/no_kl/stage1/final_model" \
  --train_offset 500 \
  --train_size 500 \
  --learning_rate 2e-6 \
  --output_dir "$run_root/no_kl/stage2"

PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python3 scripts/tinygrpo_math.py \
  --mode eval \
  --eval_model_path "$run_root/no_kl/stage2/final_model" \
  --eval_size 200 \
  --output_dir "$run_root/no_kl/eval200"
